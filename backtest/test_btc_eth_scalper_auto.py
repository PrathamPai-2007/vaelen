...
def analyze_scalper_offer_auto_cvd(filepath):
    ...
    # Scale min_cvd_notional based on average price & quantity
    avg_px = np.mean(px[:10000])
    avg_q = np.mean(qty[:10000])
    # 10000 USD equivalent
    min_cvd_notional = max(1.0, 10000.0 / (avg_px * avg_q)) if (avg_px * avg_q) > 0 else 10.0
...
if __name__ == "__main__":
    for f in ["backtest/processed/BTCUSD.npz", "backtest/processed/ETHUSD.npz"]:
        if os.path.exists(f):
            analyze_scalper_offer_auto_cvd(f)
